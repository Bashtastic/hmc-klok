...
const LOCATION_COLORS: { [key: string]: { light: string; dark: string; textColor?: string } } = {
  ...
  VLIS: { light: "hsl(45, 100%, 50%)", dark: "hsl(45, 33%, 30%)", textColor: "white" }, // Yellow with black text
};
...
const TidePhaseChart = ({ tideData }: TidePhaseChartProps) => {
  ...
  // Use the first location as reference to determine overall phase
  const phaseOffset = useMemo(() => {
    ...
    // 0% rising = just after LW, 100% rising = at HW
    // 0% falling = just after HW, 100% falling = at LW
    ...
    // At the reference bar position, we want the wave to show cycleProgress
  ...
  // The wave animates by changing the phase, making it appear to move left to right
  const barHeights = useMemo(() => {
    const heights: number[] = [];
    for (let i = 0; i < TOTAL_BARS; i++) {
      const progress = i / (TOTAL_BARS - 1);
      // Apply phase offset to animate the wave
      // angle = (progress + phaseOffset - 0.25) * 2π
